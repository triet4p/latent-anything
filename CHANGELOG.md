...
- Internal `_BMethodBase` ABC sketching the shared B-Method shape (`__call__`, `space`, `apply_trajectory`) at `methods/_b_base.py`, marked **UNSTABLE** — per Rule of Three §4a (instance #2, after Lerp and SteeringVector). Neither B-Method inherits from it; structural duck-typing only. (#sprint-11)
