...
| 32 registry entries | `uv run pytest tests/test_latent_anything/test_registry.py ... -q` (with pipeline/rollout/async) + live `GLOBAL_REGISTRY.list()` | **120 passed**; live count **32** (`analysis 10, runtime 9, adapter 8, intervention 5`) |
| 5 entry-point groups | `tests/test_plugin_groups.py tests/test_plugin_metadata.py tests/test_plugin_discovery.py` + live snapshot/expectation cross-check | **19 passed**; groups `latent_anything.{adapter,analysis,intervention,transition,planner}`, API v1 |
...
| CLI (5 commands + 2 aliases) | `tests/test_cli.py` + live `cli --help`, `capture-points --policy act` (exit 0), `list-capture-points` alias parity, `compare-runs --help` | **57 passed, 2 skipped** (with plugin groups/metadata/discovery batch) |
| Schema migrations (2) | `tests/test_portable.py tests/test_portable_results.py tests/test_run_record.py tests/test_run_record_portable.py` + live `decode_result_envelope`/`migrate_run_record` callable + portable round-trip | **25 passed** (+36 passed with disk-cache batch) |
| Negative/security | `tests/test_artifact_store.py` (traversal/symlink/checksum), `tests/test_disk_cache.py` (symlink/corrupt/oversized/key-shape), `tests/test_experiment_recorder.py` (unsafe names/secrets/oversize), portable allocation guard | behavior PASS (batches: 35+2 skipped, 36 passed) |
| Sync/async (9 pairs) | `test_runtime_async.py test_rollout_pipeline.py test_pipeline.py` + live `AnalysisPipeline.run`/`run_async` parity `True`, `RolloutPipeline.run`/`run_async` parity `True` | **120 passed** batch; live parity True |
...
| Cache (real backends) | disk + in-memory suites + live `SQLiteDiskCache` round-trip `True`, `InMemoryCache` round-trip `True` with `CacheStats(hits=1, misses=0, sets=1, size=1)` | **36 passed** batch; live True |
| Streaming (sync + async) | `tests/test_sprint75_streaming.py` + live `stream`/`stream_async` chunk counts `1`/`1` on fitted transition | **62 passed, 2 skipped** (with tracking batch) |
| Tracking backends | `tests/test_tracking_parity.py tests/test_mlflow_recorder.py tests/test_wandb_recorder.py tests/test_experiment_recorder.py` + live `LocalExperimentRecorder` run/log/finish | **62 passed, 2 skipped**; live OK (file/offline modes only, per project policy) |
