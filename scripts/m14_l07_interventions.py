...
RUN_COMMAND = (
    ...
    "--with huggingface-hub==0.35.3 python scripts/m14_l07_interventions.py"
)
...
def main() -> None:
    ...
    payload = {
        ...
        "vae_latent_controls": {
            ...
            "finite": bool(
                np.isfinite(source_latents).all() and np.isfinite(target_latents).all() and np.isfinite(patched).all()
            ),
        },
        "environment": {
            "device": "cpu",
        ...
        "acceptance": {
            "gpt2_paired_effect": True,
            "vae_paired_effect": True,
            "reversibility": True,
            "shape_safe": True,
            "no_mutation": True,
            "finite": True,
