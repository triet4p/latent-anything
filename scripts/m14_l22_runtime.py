...
def _transition() -> DeterministicLatentTransition:
    ...
    return DeterministicLatentTransition(space, 1, source_space_identity="m14-l22-runtime-fixture-v1").fit(
        states[:-1], actions, next_states
    )
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="latent-anything-m14-l22-") as temp:
        ...
        envelope = decode_result_envelope(
            encode_result_envelope(result_value, provenance={"lane": "L22"}, behavior_state={"seed": SEED})
        )
        ...
        chunks = list(
            pipeline.stream(
                initial, [actions[:2], np.empty((0, 1)), actions[2:]], max_chunk_rows=2, metadata={"episode": 3}
            )
        )
        ...
        parent = recorder.start_run(
            ...
            config={"seed": SEED},
            tags={"lane": "L22"},
            code_version="git:fixture",
            framework_version="latent-anything",
            seeds=(SEED,),
        )
        ...
        child.finish()
        resumed = recorder.start_run(
            "m14-l22",
            resume_run_id=parent.info.run_id,
            config={"seed": SEED},
            tags={"lane": "L22"},
            code_version="git:fixture",
            framework_version="latent-anything",
        ...
        recorder_ok = (
            record.status == "completed" and record.child_run_ids == (child.info.run_id,) and recorder_artifact_ok
        )
        ...
        payload_out = {
            ...
            "command": RUN_COMMAND,
            "target": {
                "filesystem": "isolated temporary Arrow/SQLite-compatible paths",
                "device": "cpu",
                "schema_versions": [
                    "portable-node-v1",
            ...
            "recorder_parent_child_resume_detail": {
                "status": str(record.status),
                "child_run_ids": list(record.child_run_ids),
                "expected_child": child.info.run_id,
                "artifact_count": len(record.artifacts),
                "artifact_ok": recorder_artifact_ok,
            },
            "checks": checks,
            "cache": {
                "sqlite_path": "<temporary>/cache/runtime.sqlite",
                "key": key,
                "disk_stats": disk.stats.__dict__
                if hasattr(disk.stats, "__dict__")
                else {
                    "hits": disk.stats.hits,
                    "misses": disk.stats.misses,
                    "sets": disk.stats.sets,
                    "entries": disk.stats.entries,
                    "bytes": disk.stats.bytes,
                },
                "in_memory_stats": {
                    "hits": pipeline_cache.stats.hits,
                    "misses": pipeline_cache.stats.misses,
                    "sets": pipeline_cache.stats.sets,
                    "size": pipeline_cache.stats.size,
                },
            },
            "recorder": {
                "backend": "filesystem",
            ...
            "optional_tracking": {
                "mlflow": "external service/credentials not claimed; local filesystem contract covered",
                "wandb": "external service/credentials not claimed; local filesystem contract covered",
            },
            "acceptance": checks,
            "cleanup": (
                "Temporary Arrow/SQLite/run roots and all scratch files were removed by "
                "TemporaryDirectory; only this JSON artifact is retained."
            ),
        }
