...
def test_openvla_is_hardware_excluded_without_becoming_evidence() -> None:
    ...
    assert openvla.classification == "hardware-excluded"
    assert openvla.status == "D0"
    coverage = evidence_ledger.coverage_summary(capabilities)
    assert coverage["core"][:2] == (41, 63)
    assert coverage["overall"][:2] == (41, 64)
