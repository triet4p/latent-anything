...
## [2026-08-04] Keep dataset bridge values processor-ready until latent capture

**Decision:** Represent LeRobot v3 schemas, episode slices, sample provenance, and bounded streaming windows in bridge-owned immutable descriptors while retaining each upstream sample mapping and tensor unchanged until an explicitly captured latent is converted to the framework's NumPy boundary.
**Alternatives considered:** Convert every dataset sample to NumPy on read; duplicate LeRobot's Parquet/video decoding into a framework dataset class; or expose only untyped raw dictionaries without alignment descriptors.
**Reason:** LeRobot's policy processors consume PyTorch structures and already own normalization, temporal windows, and decoding. Converting at the dataset boundary would add copies and risk changing processor semantics, while raw dictionaries alone would lose the episode/task/timestamp alignment needed for latent analysis.
**Consequences:** Future policy-capture adapters must consume `LeRobotSample.values` or the upstream processor directly and call the explicit captured-latent boundary only after representation capture. The bridge remains dependent on canonical LeRobot metadata and dataset iteration APIs, and does not become a second storage/decoding implementation.

## [2026-08-04] ACT capture observes the first decoder query through the official action-selection path

**Decision:** Sprint 58 adds one concrete ACT adapter that captures `model.decoder` with the shared `ActivationCaptureSession` and retains the first decoder query token, the vector consumed by ACT's first `action_head` prediction. The adapter always calls LeRobot's official preprocessor, `policy.select_action`, and postprocessor; it preserves the policy action queue and returns an uncaptured action when a queued action does not execute a new decoder query. Checkpoint and dataset revisions are immutable metadata, not inferred from mutable local caches.

**Alternatives considered:** Capture backbone feature maps, capture the full decoder sequence without identifying the selected query, call `predict_action_chunk` instead of `select_action`, or build a duplicate policy/normalization wrapper.

**Reason:** The decoder output is the closest semantically justified representation to the selected action while remaining a stable module seam in LeRobot 0.6.1. Capturing the first query gives one flat latent point per policy query and avoids ambiguous batching. Using `select_action` proves the bridge follows the behavior users and LeRobot evaluation actually execute; the queue-aware uncaptured result prevents silently attributing a latent to an action that came from cached chunk state. The Rule of Three does not justify a cross-policy capture Protocol after one concrete ACT adapter; shared behavior is provided by the existing capture lifecycle only.

**Consequences:** `ACTPolicyAdapter` owns observational capture metadata and analysis results, while LeRobot continues to own policy architecture, processors, action semantics, and queue state. PCA, linear probing, control baselines, and trajectory metrics are evidence tools rather than causal claims. Intervention and environment-effect measurement remain explicitly deferred to Sprint 61, and future policy adapters may revise the capture shape only after their concrete source/architecture evidence is recorded.

## [2026-08-05] Keep Diffusion representation axes explicit and upstream-owned

**Decision:** Sprint 59 adds one concrete Diffusion adapter around LeRobot 0.6.x. It observes the global conditioning tensor entering `policy.diffusion.unet` once per executed denoising chunk and the U-Net output at every scheduler timestep. Each representation records episode step, denoising step, and diffusion timestep; action-chunk queue misses produce an action without captures. The adapter loads `DiffusionConfig`, dataset metadata, policy, and official processors through LeRobot factories and uses a dedicated `lerobot-diffusion` uv profile.
**Alternatives considered:** Capture only the final action, flatten denoising steps into episode time, call a parallel policy implementation, or add a cross-policy capture Protocol after the ACT and Diffusion cases.
**Reason:** The U-Net global condition and per-timestep output are the nearest concrete seams to the two distinct computation phases, while LeRobot's `select_action` owns observation queues, denoising, action selection, and normalization. Keeping the axes in the trace prevents a denoising trajectory from being mistaken for an environment trajectory. The Rule of Three still defers a generic policy-capture interface until a third differing adapter proves the shape.
**Consequences:** Diffusion analysis is observational only: PCA, linear probing, density, and controls do not imply causal policy effects. The public checkpoint is pinned to `LeTau/diffusion_aloha_insertion@6126e33` with `lerobot/aloha_sim_insertion_human_image@d93d36a` and `AlohaInsertion-v0`; environment intervention remains Sprint 61 scope.

## [2026-08-06] SmolVLA captures four official module seams and one additive action-expert intervention

**Decision:** Sprint 60 adds one SmolVLA adapter around LeRobot 0.6.x. It captures the SigLIP vision encoder, the language embedding table, the state projection, and the action-expert final norm on the official `preprocess → select_action → postprocess` path, with per-capture token/modality metadata (camera, token counts, prefix offsets, denoising step). One bounded intervention adds `strength * direction` to the action-expert representation at every denoising step, with shape/finiteness/bounded-strength validation, an exception-safe hook session, and bit-exact no-change identity at strength zero. The pinned pair is the model card's own training pair: `lerobot/smolvla_libero@31d453f7edd78c839a8bbc39744a292686daf0de` + `lerobot/libero@a1aaacb7f6cd6ee5fb43120f673cebb0cfea7dd4` for `libero` / `libero_spatial`, with the dataset→policy camera rename map passed to the official `make_policy` factory.

**Alternatives considered:** Capture VLM text-model hidden states, capture the final velocity prediction, implement a generic policy-intervention protocol, or evaluate on a dataset whose camera names already match the policy config.

**Reason:** The expert norm output is the closest semantically justified representation to the selected action while remaining a stable module seam in LeRobot 0.6.1, and the four context seams reproduce the model's own prefix layout. The official factories already own renaming, tokenization, normalization, denoising, and queueing; passing the rename map through `make_policy` keeps the loader upstream-sanctioned rather than constructing a renamed metadata clone. The Rule of Three still defers a cross-policy capture/intervention interface until a third differing adapter proves the shape; shared behavior remains the existing capture lifecycle.

**Consequences:** The adapter owns observational capture metadata, the bounded intervention, and measurement summaries, while LeRobot owns policy architecture, processors, action semantics, and queue state. The claim is observational plus bounded-intervention: action change, off-target decomposition (against the direction induced through the policy's own `action_out_proj`), representation drift, and prompt/camera sensitivity are measurements, not causal claims. Environment-level causal effects remain deferred to Sprint 61. The marked public lane requires CUDA (`lerobot-smolvla` profile) and is documented in `docs/LEROBOT_INTEGRATION.md`.

## [2026-08-07] Causal simulation benchmark is a four-condition controlled environment experiment with declared disagreement rules

**Decision:** Sprint 61 evaluates the SmolVLA action-expert intervention by its controlled behavioral effect in the LIBERO `libero_spatial` simulation under four predeclared conditions: `no_hook` (official preprocess → `select_action` → postprocess with no capture hooks), `baseline` (hooks with strength-zero intervention, bit-exact identity), `random` (seeded unit-norm expert direction), and `targeted` (the expert direction inducing the largest change along one declared action axis through the policy's own `action_out_proj`). Every condition replays the same seeds and initial states with identical fixed noise; the harness (new `lerobot_benchmark.py` module) owns only the experiment protocol and bridge-owned results, while LeRobot owns the environment, observation processors, policy, and processors. Success, return, action deviation against the no-hook trajectory, per-query latency, and Wilson confidence intervals are reported per condition; offline explanation scores are correlated with environment effects and checked against declared overstatement/understatement/reversal rules. Promotion of the causal-intervention capability to D3 is conditional on a predeclared acceptance gate: baseline bit-exactness, no-hook success equality, measurable intervention action change, and all episodes within budget.

**Alternatives considered:** (a) Reuse upstream `lerobot-eval` verbatim (no intervention injection point); (b) evaluate interventions only as immediate action deltas without environment rollouts; (c) run all conditions inside one shared env instance without reset; (d) add a third policy (ACT/Diffusion) to the same benchmark.

**Reason:** Upstream evaluation cannot inject the Sprint 60 intervention, so the harness replicates the upstream rollout structure around `make_env`/`get_env_processors`/`preprocess_observation` while keeping environment control upstream-owned. Environment rollouts are the only way to demonstrate when an intervention improves, harms, or leaves behavior unchanged, which is the stated defense against "explain for completeness." Sharing one environment object would silently advance the LIBERO initial-state index between conditions and break comparability, so each condition creates a fresh environment object and resets it with the same seed. ACT and Diffusion have no intervention surface, so only SmolVLA can carry environment-level evidence, and the D3 gate is conditional on the acceptance checks rather than on judgment.

**Consequences:** `lerobot_benchmark.py` becomes the reproducible protocol owner; the `lerobot-smolvla` profile now includes LeRobot's Linux-only `libero` environment extra; the real lane requires CUDA and runs through the remote CUDA server. If the acceptance gate fails, the causal-intervention capability stays at its prior tier and a negative-result artifact is committed instead of a promotion.

## [2026-08-09] Freeze the local run-record contract before external tracking backends

**Decision:** Freeze Sprint 62's concrete `RunRecord`/`FileSystemRunRecorder` contract for local reproducibility, while deferring external tracking adapters and backend protocols to Sprint 76.
**Alternatives considered:** Introduce an abstract recorder protocol now; adopt MLflow/W&B as the primary storage contract; or keep each LeRobot evidence script with a private JSON shape.
**Reason:** Dataset inspection, policy capture/intervention, and evaluation now exercise the same record lifecycle, identity, artifact, provenance, and comparison needs, satisfying the Rule of Three for a local seam. No external backend has yet supplied evidence for the smallest portable operation set, while a private per-script schema would prevent reproducible cross-run comparison.
**Consequences:** Future local evidence must remain readable through schema-v1 migration and content-addressed artifacts. Sprint 76 may add adapters only after comparing real backend semantics against this contract; it must not silently replace or widen the local record shape.

## [2026-08-09] Reconcile LeRobot capture adapters without freezing a cross-policy protocol

**Decision:** Keep ACT, Diffusion, and SmolVLA as concrete capture adapters and do not extract a shared public or internal `Protocol` at this milestone. The three implementations share the official preprocess/select/postprocess lifecycle and metadata discipline, but their proven capture contracts differ materially: ACT observes one decoder query token; Diffusion observes conditioning plus per-timestep denoising tensors with a flattened action-chunk coordinate; SmolVLA observes four multimodal seams and supports an expert-space intervention at every denoising step.
**Alternatives considered:** Freeze a common capture-adapter interface and migrate all three, or keep the Rule-of-Three deferral undocumented.
**Reason:** The third adapter proves a recurring lifecycle, not one differing-philosophy capture family. A common protocol would either erase the Diffusion multi-axis contract and SmolVLA intervention semantics or grow policy-specific optional fields until it became a leaky abstraction. The stable reusable shape remains concrete helper behavior, while backend protocols are intentionally deferred to Sprint 76 after real tracker semantics are compared.
**Consequences:** Future adapter additions must first demonstrate which part of the lifecycle is genuinely common and which representation/intervention semantics remain policy-specific. A fourth adapter with the same capture philosophy may justify extracting a minimal lifecycle helper; it must not silently broaden this decision into a public cross-policy protocol.

## [2026-08-11] Keep the first latent transition concrete and affine-residual

**Decision:** Sprint 63 uses one concrete `DeterministicLatentTransition` for flat Euclidean states with action-conditioned affine residual dynamics, and does not introduce a transition protocol or ABC.
...
## [2026-08-11] Keep reward/value heads concrete and evaluation semantics explicit

**Decision:** Sprint 67 adds one concrete linear state/action reward scorer and one concrete finite-horizon Monte-Carlo value estimator. The evaluator freezes only the result and diagnostic semantics: masked discounted returns, terminal/padding handling, declared discount/horizon/policy/data provenance, value calibration, Bellman residuals, and real-versus-imagined score bias. The structural scorer/estimator shapes remain internal and unfrozen until materially different head instances prove a shared interface.
...
## [2026-08-26] Keep 0.1.0b1 metadata until the 0.9.0 pre-stable gates pass

**Decision:** Treat `0.9.0` as the planned pre-stable API-freeze release/compatibility epoch after Sprint 78, keep package metadata at `0.1.0b1` until all Sprint 78 gates and the release workflow are verified, prohibit a `v0.9.0` tag or publication while GitHub Actions account access is blocked, and retain `1.0.0` stable publication as Sprint 80's stop-before-release target.
**Alternatives considered:** Bump package metadata and tag `v0.9.0` immediately at the end of Sprint 78, treat `0.9.0` as the stable release, or publish despite the unresolved external Actions-account blocker.
**Reason:** The repository's current beta metadata and M14 stop conditions must remain truthful while Sprint 78 completes compatibility/API-freeze work and Sprint 79 supplies real-system evidence; the owner explicitly requires no release signal that could be mistaken for verified publication readiness.
**Consequences:** Documentation may describe the planned `0.9.0` compatibility epoch, but no metadata/tag/publish change is allowed before the gates and workflow are verified. Sprint 80 remains responsible for the `1.0.0` stable decision, and future agents must preserve the stop-before-release rule unless this ADR is amended.

## [2026-08-26] Retain Any only at heterogeneous runtime boundaries

**Decision:** Retain `Any` only for genuinely heterogeneous registry-factory, metadata/provenance, optional-backend, and constructor-`**kwargs` seams, while narrowing concrete known configuration parameters such as `compare_probes.linear_config` and excluding literal-string scan false positives from the typed count.
**Alternatives considered:** Introduce speculative `TypedDict`/`Protocol` schemas for provider metadata and dynamic factories, leave all `Any` annotations unchanged, or treat textual occurrences of `Any` as typed API debt.
**Reason:** The 78.32 audit found stable concrete config types for the linear probe but no single schema for heterogeneous provider payloads and backend constructors. Narrowing the known config seam improves strict typing without inventing abstractions before three differing implementations; retaining the other boundaries preserves provider extensibility and the NumPy/JSON public contract. A literal title string is not a type annotation and must not distort the API inventory.
**Consequences:** Future API-freeze audits must classify new `Any` uses by boundary and require an owner decision before replacing justified heterogeneous seams. Concrete config additions should use their established model types, while provider-specific metadata and kwargs remain deliberately open until a real shared schema is demonstrated.

## [2026-08-26] Freeze the Sprint 78 public compatibility surface

**Decision:** After Sprint 78, the 205 runtime-export/202 canonical-stable surface and its documented contracts require reviewed compatibility handling; do not remove aliases, bump the version, create a tag, publish, or claim release readiness from this checkpoint.
...
## [2026-09-17] Make diagnostic depth the 1.0 release contract

**Decision:** Close Sprint 79 with `0.9.0` as an honest pre-stable API/evidence baseline, move the former stable-publication Sprint 80 to Sprint 81, and use the new Sprint 80 to prove an end-to-end representation-diagnostic loop for ordinary deep-learning models; broad theory percentages and heavyweight model-family coverage are no longer stable-release gates.
