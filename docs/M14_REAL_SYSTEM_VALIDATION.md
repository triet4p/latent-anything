...
|---|---|---|---|---|---|---|---|
| L01 | LatentSpace, LatentValue, PipelineContract, Result | `latent_space.py`, `latent_value.py`, `pipeline.py` → core unit tests, D1 | sklearn digits split; NumPy/PyTorch CPU | local; `uv run pytest tests/test_latent_anything/test_latent_space.py tests/test_latent_anything/test_latent_value.py -q` | finite shapes/dtypes, stable schema, no input mutation; `artifacts/m14/l01-core.json` | <1 GB, offline; remove temp files | planned; no blocker; API owner |
| L02 | geometry: covariance, projection, SO3/SE3, DTW, smoothing | `geometry.py`, `projection.py`, `pose.py`, `dtw.py`, `temporal.py` → focused tests, D1/D2 synthetic | real recorded trajectory substitute: `tests/fixtures/` + sklearn digits labels; NumPy/SciPy CPU | local; `uv run pytest tests/test_latent_anything/test_covariance_geometry.py tests/test_latent_anything/test_projection.py tests/test_latent_anything/test_pose.py tests/test_dtw.py tests/test_temporal.py -q` | metric invariants, valid rotations, DTW path, boundary/smoothing thresholds; `artifacts/m14/l02-geometry.json` | <1 GB, offline; delete generated plots | planned; 3D external backends absent; geometry owner |
| L03 | analysis: KMeans, probes, feature ranking | `clustering.py`, `probes.py`, `mlp_probe.py` → analysis tests, D1 | GPT-2 hidden states on `gpt2@e7da7f221d5bf496a4811970ad59b19a5b3ff2a4`; sklearn digits labels | local CPU first, optional remote; `uv run pytest tests/test_clustering.py tests/test_probes.py -q` | seeded train/holdout metrics, baseline and memorization controls; `artifacts/m14/l03-analysis.json` | ~2–4 GB CPU, model download/cache; purge disposable HF cache | planned; model license/access capture; analysis owner |
| L04 | IntegratedGradients, TCAV, sensitivity/intervention controls | `integrated_gradients.py`, `tcav.py` → focused tests, D1 | GPT-2 `@e7da7f2`; real prompt/label fixture | local CPU; `uv run pytest tests/test_integrated_gradients.py tests/test_tcav.py -q` | completeness/sensitivity, selectivity, seeded CI and causal intervention; `artifacts/m14/l04-explanations.json` | ~2–4 GB, network on first run; retain manifest, delete weights if disposable | planned; D3 absent until run; explanation owner |
...
| L11 | TransformerLogitTarget, hidden-state adapter | `integrations/transformer_lm.py` → transformer tests, D2 candidate | `gpt2@e7da7f221d5bf496a4811970ad59b19a5b3ff2a4`, MIT; Transformers | local CPU; `uv run pytest tests/test_transformer_lm_network.py -m network -q` | vocab 50257, 13 layers, hidden 768, logits/hidden parity; `artifacts/m14/l11-gpt2.json` | ~2–4 GB; public download; remove disposable cache | planned; capture versions/license; transformer owner |
