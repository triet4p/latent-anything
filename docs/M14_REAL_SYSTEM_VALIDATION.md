...
| L19 | OpenVLA policy/latent capture and intervention | No OpenVLA adapter exists yet; contract is recorded in [`l19-openvla.config.json`](../artifacts/m14/l19-openvla.config.json) → no executable local test | `openvla/openvla-7b-finetuned-libero-spatial@962318cec55ac10993ff0f5f43eda9a270b4c873` (base `openvla/openvla-7b@47a0ec7fc4ec123775a391911046cf33cf9ed83f`); modified RLDS `openvla/modified_libero_rlds@6ce6aaaaabdbe590b1eef5cd29c0d33f14a08551`; LIBERO `@8f1084e3132a39270c3a13ebe37270a43ece2a01` / `libero_spatial` | authorized Linux NVIDIA CUDA host only; implement/review official Transformers `AutoProcessor`/`AutoModelForVision2Seq` path, then run pinned OpenVLA network lane in disposable clone | 7 finite action values, exact preprocessing/token/action postprocessing, 500 LIBERO-Spatial trials, paired intervention/control and revision manifest; `artifacts/m14/l19-openvla.json`; blocked receipt [l19-openvla.json](../artifacts/m14/l19-openvla.json); config [l19-openvla.config.json](../artifacts/m14/l19-openvla.config.json) | 15,082,474,368-byte BF16 checkpoint; upstream reference is NVIDIA A100 (80GB used for fine-tuning); network/download and isolated-cache cleanup required; model card says MIT while upstream repository warns Llama-2-derived weights inherit Llama Community License | blocked; this Windows host has no CUDA, checkpoint is not cached, network opt-in is disabled, and no OpenVLA capture adapter is implemented |
...
required for current evidence; the current closure from that fresh execution is
recorded below. The companion
...
### Current L04.8 Disentanglement closure

The single owner-authorized direct PowerShell `ssh.exe` execution on pushed SHA
`4d3a4b6551d6091ce96c73a704e642867c2f2580` completed the real
`TransformerLMIntegration` CUDA path. The Disentanglement record is
`passed_real_cuda`, accepted, evidence-eligible, and D2: factor permutation,
group-preserving shuffle, target-token-excluded raw-token baseline, seeded
repeat, and the strict held-out gain gate all passed. The run used an RTX 4060
Ti and recorded `19.096944198012352` seconds, `529430016` allocated bytes,
`572522496` reserved bytes, and `1604595712` RSS bytes.

The retained triplet is [`l04-explanations.Disentanglement.attempt2.partial.json`](../artifacts/m14/l04-explanations.Disentanglement.attempt2.partial.json),
[`l04-explanations.Disentanglement.attempt2.run.json`](../artifacts/m14/l04-explanations.Disentanglement.attempt2.run.json),
and [`l04-explanations.Disentanglement.attempt2.failure.json`](../artifacts/m14/l04-explanations.Disentanglement.attempt2.failure.json); the sanitized
[`final retention audit`](../artifacts/m14/l04-explanations.ssh.Disentanglement.4d3a4b6551d6091ce96c73a704e642867c2f2580.audit.json)
records archive SHA-256 `c222475e9591eed5fbc45f6202aff7edaf83926a8fa7a6a0e85699aacce614d0`,
member hashes, validator PASS, and `deleted_verified` raw finalization. The
final raw was `266091` bytes with SHA-256
`7b102d4a99c3b89c6e6da0becc694f9baa04d9a42fef8410b137136db2ea238d` and is
absent after verified quarantine deletion. This is the only current
repository-promotion claim for this capability; the earlier timeout and
retention-failure captures remain historical, non-promoting sidecars: the
[`transport-timeout sidecar`](../artifacts/m14/l04-disentanglement.732d819338b4b06ac9f2d672b9dc46c2d8149e99.attempt1.transport-failure.json)
and [`strict-retention-failure sidecar`](../artifacts/m14/l04-disentanglement.0c7cfa208bc7e9c9d4c1f848f76a0ac189735b19.attempt1.retention-failure.json).
