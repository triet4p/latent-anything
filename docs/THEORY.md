...
- [x] **Gaussian set operations** — add, remove, merge Gaussian. Tại sao explicit representation amenable với world model hơn implicit NeRF. Evidence D2: constrained rigid/opacity/color/remove/merge operations and held-out multi-view metrics in Sprint 55.
...
- [x] **Density estimation trong latent** — Gaussian mixture model, normalizing flow trên latent. Dùng để in/out-of-distribution detection.
- [x] **Optimal transport trong latent** — Wasserstein distance giữa hai distribution latent. Dùng để compare trajectory distribution.
...
- [x] **Superposition hypothesis** — model encode nhiều feature hơn số neuron bằng superposition. Tại sao PCA không đủ để decompose.
...
- [x] **Stochastic transition** — f(z_t, a_t) → p(z_{t+1}). Gaussian output. Khi nào cần stochastic.
- [x] **RSSM — Recurrent State Space Model (Dreamer)** — tách deterministic component (GRU) và stochastic component (Gaussian). Tại sao split này giúp stability và long-horizon prediction.
- [x] **Kalman filter và variants** — linear Gaussian state space model. Nền tảng lý thuyết cho stochastic transition. Extended KF, Unscented KF cho nonlinear.
- [x] **Latent trajectory** — sequence z_0..T trong latent space. Operations: smoothing, segmentation, similarity, interpolation giữa trajectory.
- [x] **Rollout / latent imagination** — lặp z_{t+1} = f(z_t, a_t) nhiều step. Cost O(k·d) vs O(k·H·W·C) trong pixel space.
- [x] **Trajectory similarity metrics** — DTW (Dynamic Time Warping), Fréchet distance. So sánh trajectory có length khác nhau.
...
- [x] **Scaling Monosemanticity (Anthropic, 2024)** — scale SAE lên Claude 3 Sonnet. Cách evaluate feature quality.
- [x] **Probing classifiers survey (Belinkov, 2022)** — tổng quan các probing method. Khi nào dùng gì.
- [x] **UMAP theory (McInnes et al., 2018)** — fuzzy topological structure. Tại sao UMAP preserve global structure tốt hơn t-SNE.
- [x] **PaCMAP** — alternative UMAP, balance local và global structure tốt hơn.
...
- [x] **Lie groups và Lie algebra** — rotation group SO(3), SE(3). Cần cho 3DGS covariance parameterization và robotics.
- [x] **Spherical harmonics** — basis functions trên sphere, SH coefficients, addition theorem, real SH. Hàm riêng Laplace–Beltrami, Funk–Hecke và bất biến quay; nền toán cho 3DGS color encoding.
