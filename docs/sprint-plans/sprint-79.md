...
- [ ] Publish the `0.9.0` pre-stable evidence baseline: reconcile the new depth-first scope across authoritative docs and ADRs, make an explicit reviewed compatibility-alias decision, update metadata/changelog/migration/release notes, build and verify wheel/sdist from a clean checkout, run the audited release workflow, create the `v0.9.0` tag/release, and record post-publication verification. This release must preserve all negative evidence and must not claim `1.0.0` diagnostic readiness.
