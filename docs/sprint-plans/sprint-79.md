...
- [ ] Execute the exhaustive supported-scope [theory evidence-gap plan](../EVIDENCE_GAP_PLAN.md) and its row-level [machine-readable map](../../artifacts/task_78.38_gap_map.json); keep D0/D1 statuses unchanged until validator-backed D2/D3 artifacts exist. `THY-X01-OPENVLA` remains a historical D0 feasibility record but is removed from the active queue and coverage denominator under the permanent 16 GiB hardware ceiling.
...
- [ ] Execute explanation-validity controls and confirm the scoped theory ledger meets 95% core / 90% overall D2-or-D3 thresholds. Hardware exclusion changes only the non-core overall denominator from 65 to 64; the 63-row core denominator and every empirical acceptance gate remain unchanged.
...
- [ ] Measure performance budgets and LeRobot policy overhead against Sprint 77 gates for the supported 16 GiB execution scope. Canonical OpenVLA timing is not an active requirement and must not be replaced by quantized/offloaded evidence under the BF16 contract.
...
- [ ] Fix only release blockers, rerun the complete affected matrix, reconcile docs/ledger conflicts, and cut the release candidate only after the external GitHub Actions account is available. **Hold:** [`task_79_line603_release_blocker_hold.md`](../../artifacts/task_79_line603_release_blocker_hold.md) records that Actions availability is demonstrated, but 19 core / 17 scoped-overall evidence shortfalls and the remaining M14/explanation blockers remain. Owner action: close those supported-scope gates with exact reviewed artifacts, rerun affected matrices, reconcile the ledger, then run the authorized `triet4p` release workflow; no tag, publication, or version bump now.

## Notes / Blockers

No percentage waiver may hide an implementation-applicable core theory gap. Any exclusion must be classified and justified in the Sprint 27 ledger. The supported release-evidence hardware ceiling is 16 GiB; therefore L19/OpenVLA canonical BF16 execution is removed from the active Sprint 79, M14, and release-gate scope because its measured contract requires unavailable >=24 GiB hardware. Its D0 config and feasibility receipt remain immutable historical records, not a pass or waiver, and no OpenVLA capability, performance, or quality claim is authorized. Reintroduction requires an explicit future plan revision and adequate hardware. SAM, OpenCLIP, timm, Torchvision model adapters, Open3D, trimesh, and unnamed 3DGS are not stable API claims; they remain explicit backlog/blocker rows.

Sprint 78.38 records the starting denominator and all 40 historical gap
records. The active queue now has 39 records after the hardware exclusion, and
the scoped map records **41/63 core and 41/64 overall qualifying rows** after
the L02 trajectory-similarity and queue-position-39 dictionary-learning
reconciliations. The core denominator is unchanged.
